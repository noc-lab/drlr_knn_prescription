...
from drlr_knn_prescription.transform import DRLRTransformer
...
def fit_drlr_knn_submodels(x, y, num_neighbor, reg_l2, random_seed=0):
    ...
    transformer = DRLRTransformer(solver='gurobi', reg_l1=0, reg_l2=reg_l2)
    informed_knn = Pipeline(
        [('transformer', transformer), ('knn', KNeighborsRegressor(n_neighbors=num_neighbor))])
...
def obtain_best_model(data, best_parameters):
    ...
    for i in range(num_prescription):
        ...
        num_neighbor = int(rho * int(np.sqrt(len(y))))
        transformer = DRLRTransformer(solver='gurobi', reg_l1=0, reg_l2=best_parameters[i][0])
...
def main():
    ...
    pickle.dump(model_collection, open(model_save_dir + 'drlr_knn_trial_' + str(trial_number) + '.pkl', 'wb'))
    pickle.dump(model_imputation_conf,
                open(model_save_dir + 'drlr_knn_impute_trial_' + str(trial_number) + '.pkl', 'wb'))
